...
def warmup(client, warmup_requests=13):
    dummy_data = {
        "input": "Write fibonacci series code"
      }  # or any representative input
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(functools.partial(send_request, client, dummy_data)) for _ in range(warmup_requests)]
        for future in concurrent.futures.as_completed(futures):
            _ = future.result()  # Discard results, just warming up


def send_request(client, input_data):

    try:
        start_time = time.perf_counter()  # Start the timer
        output = client.completions.create(model="/sadata/ramanr/nlp/2023/llms/hf/starcoderbase",
                                prompt=input_data['input'],
                                max_tokens=810,
                                temperature=0.7)
        end_time = time.perf_counter()  # End the timer
        return True, end_time - start_time  # Return status code and time taken
...
def main():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = [executor.submit(functools.partial(send_request, client, input_data)) for input_data in data]

        # Collect results
        results = [future.result() for future in concurrent.futures.as_completed(futures)]
        
        # Count successful and failed requests
        successful_requests = sum(1 for result in results if result[0] is True)
    ...
    csv_file = f"benchmark_{sanitized_dataset}_vllm_entrypoint.csv"
